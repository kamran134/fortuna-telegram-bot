...
export async function showUsersInSelectedGroup(query: CallbackQuery, bot: any): Promise<void> {
    ...
    try {
        ...
        if (users && users.length > 0) {
            const usersString = users.map((user: any) =>
...
export async function showLastUserInSelectedGroup(query: CallbackQuery, bot: any): Promise<void> {
    const adminChatId = query.message?.chat.id;
    const selectedGroupChatIdStr = query.data?.split('_')[1];
...
export async function searchUserInSelectedGroup(query: CallbackQuery, bot: any): Promise<void> {
    const adminChatId = query.message?.chat.id;
    const selectedGroupChatIdStr = query.data?.split('_')[1];
