...
export async function getGames(pool: Pool, chatId: number): Promise<Game[]> {
    try {
...
export async function getGamesTimes(pool: Pool, chatId: number): Promise<{ game_starts: string; label: string }[]> {
    try {
...
export async function addGame(pool: Pool, chatId: number, { date, start, end, users_limit, location, label }: GameOptions): Promise<number | undefined> {
    try {
...
export async function deactiveGame(pool: Pool, gameId: number): Promise<string> {
...
export async function deleteGame(pool: Pool, gameId: number): Promise<string> {
    const client = await pool.connect();
...
export async function changeGameLimit(pool: Pool, chatId: number, { label, limit }: GameLimitOptions): Promise<string> {
    const client = await pool.connect();
...
export async function checkGameStatus(pool: Pool, gameId: number): Promise<boolean> {
    const client = await pool.connect();
