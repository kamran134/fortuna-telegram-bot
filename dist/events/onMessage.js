import { register, showMenu, getRegistered, startGame, showGames, deactiveGames, getGamePlayers, addGuest, whatTime, agilliOl, getAzList, tagGamePlayers, changeGameLimit, connectTo, showGroups, showYourGroups, addJoke } from "../commands/index.js";
...
export const onMessage = async (msg, bot) => {
    ...
    if (messageText === '/register')
        register({ chatId, user }, bot);
    else if (messageText === '/menu')
        showMenu(msg, bot);
    else if (messageText === '/tagregistered')
        getRegistered(msg, bot, 'tag', isAdmin);
    else if (messageText === '/showregistered')
        getRegistered(msg, bot, 'show', isAdmin);
        ...
        showGames(chatId, bot);
    else if (messageText === '/deletegame') { }
    else if (messageText === '/deactivegame')
        deactiveGames(msg, bot, isAdmin);
    else if (messageText === 'приффки')
        bot.sendMessage(chatId, 'ПрИфФкИ, ' + msg.from.first_name + '. КаК дЕлИфФкИ. (Что за ванилька из начала нулевых?)');
    else if (messageText === 'привет')
        bot.sendMessage(chatId, 'Привет, ' + msg.from.first_name + '. Играть будем?');
    else if (messageText === '/list')
        getGamePlayers(chatId, bot);
    else if (messageText === 'Пока')
        bot.sendMessage(chatId, 'До свидания, ' + msg.from.first_name);
        ...
        bot.sendMessage(chatId, `Нет, ${msg.from.first_name}. Это заклинание не откроет тебе двери в админ-панель...`, { reply_to_message_id: msg.message_id });
        ...
        tagGamePlayers(chatId, bot, isAdmin);
    else if (messageText.startsWith('/changelimit') && isAdmin)
        changeGameLimit(msg, bot);
    else if (messageText.startsWith('/changelimit') && !isAdmin)
        ...
        tagUndecidedPlayers(chatId, bot);
    else if ((messageText === 'Бот, растормоши неопределившихся' || messageText === '/tagundecided') && !isAdmin)
    ...
    // for admin group
        ...
        showYourGroups(chatId, bot, 'PayList');
